...
description: Etapa final de qualquer planejamento. Normaliza a origem do plano (conversa, docs_dev ou .context/plans) em um diretório próprio dentro de .context/plans/{feature}/, levanta libs internas e serviços externos, coleta documentação atual (context7 + WebSearch/WebFetch) em info_aux_{feature}.md, reestrutura o plano completo, e por fim cria o plano canônico via MCP ai-context (scaffoldPlan + workflow-init) referenciando esses arquivos e deixando o workflow pronto para implementação. Na conclusão, consolida o canônico dentro da pasta e move tudo para archive/.
phases: [P]
...
- **(b) Em `docs_dev/`** — ex.: `docs_dev/planejamento/.../plano_xyz.md`.
...
#### Grupo A — Pacotes/Libs Python (gerenciados pelo projeto)

Cruze com o manifest do projeto para capturar versões fixadas:
...
Identifique **qualquer coisa fora do código Python** com que o plano interage:
...
- **Plataformas SaaS** acessadas via HTTP/SDK não-Python
...
Agent({
  ...
  prompt: `
    ...
    Plano base (origem: conversa / docs_dev / .context):
    <conteúdo-base>
    ...
    3. Corrija integrações externas: endpoints incorretos, headers faltando,
       formatos de body errados, autenticação desatualizada.
    4. Enriqueça as etapas de integração com exemplos de código atualizados
       extraídos do info_aux.
    5. Respeite a arquitetura (Multi-Tenant + Result Pattern — ver
       .context/docs/architecture.md). Não invente libs novas sem necessidade.
    6. Estruture o plano em FASES claras (cada uma mapeável a uma fase PREVC e a
       um agente especialista), pois servirão de base ao plano canônico do MCP.
    7. Adicione seção "Correções aplicadas": o que mudou, por quê e a fonte.
...
### 5. Canonização no MCP ai-context (sessão principal)
...
3. Se a origem era `docs_dev/`, deixe claro que a fonte da verdade agora é o
   plano canônico em `.context/plans/` (o arquivo em `docs_dev/` vira histórico).
...
- [ ] Origem do plano identificada (conversa / docs_dev / .context) e conteúdo-base obtido.
- [ ] Slug `{feature}` definido e diretório `.context/plans/{feature}/` criado.
- [ ] Grupo A (libs Python) levantado com versões do `pyproject.toml`.
