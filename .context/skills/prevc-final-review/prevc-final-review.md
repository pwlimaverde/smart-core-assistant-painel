...
- Logo no início da fase **C (Confirmation)**, **antes** da etapa de arquivamento
  do plano descrita em [prevc-confirmation](../prevc-confirmation/prevc-confirmation.md).
...
- Padrões do projeto: CLAUDE.md, AGENTS.md, .context/docs/architecture.md
...
2. Audite o código alterado contra os padrões: Result Pattern, Multi-Tenant
   (user.tenant), type hints (pyright strict), docstrings Google, sem secrets
   hardcoded, sem N+1, sem código morto/TODO esquecido.
...
4. Revalide após corrigir:
     ...
     uv run task test-docker   (se houver testes; não criar testes novos)
...
Ao terminar, o agente principal retoma [prevc-confirmation](../prevc-confirmation/prevc-confirmation.md)
