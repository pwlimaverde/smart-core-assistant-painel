...
def mark_read(
    ...
    3. Dispara ``POST /message/markread`` no Evolution Go (best-effort, throttled).
    ...
    # Task 2.3 / 2.6 — dispara markread no Evolution Go (best-effort + throttle)
    try:
...
def _dispatch_evolution_markread(atendimento_id: int) -> None:
    """Chama ``POST /message/markread`` no Evolution Go para o atendimento.

    Throttle: 1 chamada por atendimento a cada 60 segundos via Django cache.
    Se o adapter for v2, é no-op (v2 não suporta markread nativamente).
    ...
    _THROTTLE_KEY = f"evo_markread_throttle_{atendimento_id}"
    if cache.get(_THROTTLE_KEY):
        return  # throttled — já foi chamado nesta janela
    cache.set(_THROTTLE_KEY, 1, timeout=60)
    ...
    # Coleta IDs das mensagens não lidas do contato
    from smart_core_assistant_painel.app.atendimentos.models import (
        Mensagem,
        TipoRemetente,
    )
