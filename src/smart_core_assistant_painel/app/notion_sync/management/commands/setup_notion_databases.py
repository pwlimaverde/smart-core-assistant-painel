...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        self.stdout.write(self.style.SUCCESS('Iniciando setup das databases do Notion...'))
        ...
        try:
            ...
            self._setup_departamento_database(database_id, data_source_id, update)

            # Configuração para AtendenteHumano
            self._setup_atendente_humano_database(database_id, data_source_id, update)

            self.stdout.write(
                self.style.SUCCESS('Setup das databases concluído com sucesso!')
            )
    ...
    def _print_database_info(self, config: NotionDatabaseConfig) -> None:
        ...
        self.stdout.write(f"Sincronização: {'Habilitada' if config.sync_enabled else 'Desabilitada'}")
