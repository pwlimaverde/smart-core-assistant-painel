...
class TenantRegistrationForm(forms.Form):
    ...
    slug = forms.SlugField(
        ...
        help_text=_(
            "O endereço será https://[slug].smartcoreassistant.com.br"
        ),
        widget=forms.TextInput(
            attrs={
    ...
    def clean_slug(self):
        ...
        if tenant:
            # Se for tenant ativo/concluído, bloqueia.
            if tenant.setup_completed:
                raise ValidationError(_("Este subdomínio já está em uso."))
            # Se for pendente, permite passar (Resumption no Service)
