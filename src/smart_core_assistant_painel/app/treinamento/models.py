...
class QueryTestFeedback(models.Model):
    """[TRN-TEST-003] Registra feedback de testes de query.

    Armazena resultados de testes manuais do bot para melhoria contínua.
    """

    id: models.AutoField = models.AutoField(primary_key=True)
    mensagem_original: models.TextField[str] = models.TextField(
        help_text="Mensagem enviada pelo usuário no teste",
    )
    resposta_bot: models.TextField[str] = models.TextField(
        help_text="Resposta gerada pelo bot",
    )
    resposta_corrigida: models.TextField[str | None] = models.TextField(
        blank=True,
        null=True,
        help_text="Resposta corrigida pelo usuário (se avaliação ruim)",
    )
    avaliacao: models.CharField[str] = models.CharField(
        max_length=10,
        help_text="Avaliação: 'bom' ou 'ruim'",
    )
    confiabilidade: models.FloatField[float] = models.FloatField(
        default=0.0,
        help_text="Score de confiabilidade (0.0 a 1.0)",
    )
    entidades_json: models.JSONField[dict[str, Any]] = models.JSONField(
        default=dict,
        blank=True,
        help_text="Entidades extraídas da mensagem",
    )
    intents_json: models.JSONField[dict[str, Any]] = models.JSONField(
        default=dict,
        blank=True,
        help_text="Intents detectados na mensagem",
    )
    documentos_ids: models.JSONField[list[int]] = models.JSONField(
        default=list,
        blank=True,
    ...
    def __str__(self) -> str:
        return (
            f"Feedback #{self.id} - {self.avaliacao} "
