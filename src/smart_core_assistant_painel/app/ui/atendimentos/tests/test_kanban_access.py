...
from smart_core_assistant_painel.app.ui.atendimentos.models import Atendimento, StatusAtendimento
from smart_core_assistant_painel.app.ui.clientes.models import Contato


@pytest.mark.django_db
def test_non_member_cannot_access_other_department(client):
    d1 = Departamento.objects.create(nome="Depto A", ativo=True, api_key="key-a", telefone_instancia="5511999999001")
    d2 = Departamento.objects.create(nome="Depto B", ativo=True, api_key="key-b", telefone_instancia="5511999999002")

    user = User.objects.create_user(username="userA", password="pass")
    AtendenteHumano.objects.create(nome="Agente A", usuario_sistema="userA", departamento=d1, ativo=True, disponivel=True)

    assert client.login(username="userA", password="pass")

    # Access own department OK
    url_d1 = reverse("atendimentos:kanban_departamento", kwargs={"departamento_id": d1.id})
    resp1 = client.get(url_d1)
    assert resp1.status_code == 200
    assert f"Kanban - {d1.nome}" in resp1.content.decode()

    # Access other department denied
    url_d2 = reverse("atendimentos:kanban_departamento", kwargs={"departamento_id": d2.id})
...
def test_transfer_blocked_for_non_member(client):
    ...
    d2 = Departamento.objects.create(nome="Depto B", ativo=True, api_key="key-b", telefone_instancia="5511999999002")
    user = User.objects.create_user(username="userA", password="pass")
    agente_a = AtendenteHumano.objects.create(nome="Agente A", usuario_sistema="userA", departamento=d1, ativo=True, disponivel=True)
    contato = Contato.objects.create(nome_contato="Cliente X", telefone="5511998887777")
...
def test_partial_modal_lists_only_allowed_departments(client):
    ...
    url_d1 = reverse("atendimentos:kanban_departamento", kwargs={"departamento_id": d1.id})
    resp = client.get(url_d1, data={"partial": "1", "atendimento_id": atendimento.id})
    assert resp.status_code == 200
    content = resp.content.decode()
