...
from smart_core_assistant_painel.app.ui.atendimentos.signals import (
    mensagem_bufferizada,
    signal_agendar_processamento_mensagens,
)
...
class TestAtendimentosSignals(TestCase):
    ...
    def test_signal_agendar_processamento_mensagens_integration(self) -> None:
        ...
        self.assertEqual(
            schedule.func,
            "smart_core_assistant_painel.app.ui.atendimentos.utils.send_message_response",
        )
    ...
    def test_signal_agendar_processamento_mensagens_exception_handling(
        ...
        with patch(
            "smart_core_assistant_painel.app.ui.atendimentos.signals.Schedule.objects"
        ) as mock_schedule_objects:
            mock_schedule_objects.create.side_effect = Exception(
                "Test exception"
            )

            # Act
            signal_agendar_processamento_mensagens(
                sender="test", phone=self.phone
            )

            # Assert
            mock_logger.error.assert_called_once()
            self.assertIn(
                "Erro ao agendar processamento",
