...
class TestSignalHandlers(TestCase):
    ...
    def test_contato_created_handler(self) -> None:
        ...
        def log_contato_created(sender, instance, created, **kwargs):
            if created:
    ...
    def test_atendimento_status_change_handler(self) -> None:
        ...
        def monitor_status_change(sender, instance, **kwargs):
            if instance.pk:
                try:
    ...
    def test_mensagem_analytics_handler(self) -> None:
        ...
        def track_mensagem(sender, instance, created, **kwargs):
            if created:
...
class TestSignalErrorHandling(TestCase):
    ...
    def test_signal_handler_exception(self) -> None:
        ...
        @receiver(post_save, sender=Contato)
        def failing_handler(sender, instance, created, **kwargs):
            if created:
    ...
    def test_signal_handler_with_database_error(self) -> None:
        ...
        def db_error_handler(sender, instance, created, **kwargs):
            if created:
                try:
        ...
        contato = Contato.objects.create(
            telefone='5510888888888',
            nome='Cliente DB Erro'
        )
...
class TestSignalIntegration(TransactionTestCase):
    ...
    def test_complete_workflow_signals(self) -> None:
        ...
        @receiver(post_save, sender=Contato)
        def log_contato(sender, instance, created, **kwargs):
            if created:
                self.integration_log.append(f"Contato criado: {instance.nome}")
        
        @receiver(post_save, sender=Atendimento)
        def log_atendimento(sender, instance, created, **kwargs):
            if created:
        ...
        @receiver(post_save, sender=Mensagem)
        def log_mensagem(sender, instance, created, **kwargs):
            if created:
    ...
    def test_signal_ordering(self) -> None:
        ...
        def pre_save_contato(sender, instance, **kwargs):
            execution_order.append("pre_save_contato")
        
        @receiver(post_save, sender=Contato)
        def post_save_contato(sender, instance, created, **kwargs):
            execution_order.append("post_save_contato")
        
        # Cria contato
        contato = Contato.objects.create(
            telefone='5510555555555',
            nome='Cliente Ordem'
        )
...
class TestSignalPerformance(TestCase):
    ...
    def test_multiple_signals_performance(self) -> None:
        ...
        @receiver(post_save, sender=Contato)
        def performance_handler(sender, instance, created, **kwargs):
            nonlocal call_count
            call_count += 1
    ...
    def test_signal_memory_usage(self) -> None:
        ...
        @receiver(post_save, sender=Mensagem)
        def memory_handler(sender, instance, created, **kwargs):
        ...
        contato = Contato.objects.create(
            telefone='5510333333333',
            nome='Cliente Memória'
        )
