...
class TestWaBuffer:
    ...
    def test_set_wa_buffer(self, mock_service_hub, mock_cache):
        ...
        message = create_message_data(
            numero_telefone="12345", conteudo="teste"
        )
...
class TestCompileMessageData:
    ...
    def test_compile_single_message(self):
        message = create_message_data(
            conteudo="conteudo 1", metadados={"m1": "v1"}
        )
...
@patch(
    "smart_core_assistant_painel.app.ui.oraculo.utils._analisar_conteudo_mensagem"
)
@patch(
    "smart_core_assistant_painel.app.ui.oraculo.utils.processar_mensagem_whatsapp"
)
@patch(
    "smart_core_assistant_painel.app.ui.oraculo.utils._compile_message_data_list"
)
@patch("smart_core_assistant_painel.app.ui.oraculo.utils.cache")
class TestSendMessageResponse:
    """Testes para a função send_message_response."""

    def test_send_message_empty_buffer(
        self,
        mock_cache,
        mock_compile,
        mock_processar,
    ...
    @patch(
        "smart_core_assistant_painel.app.ui.oraculo.models.Mensagem.objects.get"
    )
    @patch(
        "smart_core_assistant_painel.app.ui.oraculo.utils._pode_bot_responder_atendimento",
        return_value=True,
    )
    @patch(
        "smart_core_assistant_painel.app.ui.oraculo.models.Treinamentos.build_similarity_context",
        return_value="",
    )
    @patch("smart_core_assistant_painel.app.ui.oraculo.utils.SERVICEHUB")
    ...
    @patch(
        "smart_core_assistant_painel.app.ui.oraculo.models.Mensagem.objects.get",
        side_effect=utils.Mensagem.DoesNotExist,
    )
    def test_send_message_mensagem_not_found(
        self,
        mock_msg_get,
        mock_cache,
        mock_compile,
...
@patch(
    "smart_core_assistant_painel.app.ui.oraculo.utils.mensagem_bufferizada.send"
)
@patch("smart_core_assistant_painel.app.ui.oraculo.utils.SERVICEHUB")
@patch("smart_core_assistant_painel.app.ui.oraculo.utils.cache")
class TestSchedMessageResponse:
    ...
    def test_sched_message_timer_not_set(
        self, mock_cache, mock_service_hub, mock_signal_send
    ):
...
class TestAnalisarConteudoMensagem:
    ...
    def test_analise_com_sucesso(
        self, mock_filter, mock_msg_get, mock_features_compose
    ):
        ...
        mock_atendimento.carregar_historico_mensagens.return_value = {
            "conteudo_mensagens": ["oi"]
        }
        mock_features_compose.analise_previa_mensagem.return_value = (
            AnalisePreviaMensagem(intent=[{"saudacao": "oi"}], entities=[])
        )

        with patch(
            "smart_core_assistant_painel.app.ui.oraculo.utils._processar_entidades_contato"
        ) as mock_proc_entidades:
            utils._analisar_conteudo_mensagem(1)

            mock_atendimento.carregar_historico_mensagens.assert_called_once_with(
    ...
    def test_chama_apresentacao_se_novo_atendimento(
        self, mock_filter, mock_msg_get, mock_features_compose
    ):
        mock_atendimento = MagicMock(spec=Atendimento, id=1)
        mock_mensagem = MagicMock(spec=Mensagem, atendimento=mock_atendimento)
        mock_msg_get.return_value = mock_mensagem
        mock_atendimento.carregar_historico_mensagens.return_value = {}
        mock_filter.return_value.exclude.return_value.exists.return_value = (
            False
        )
    ...
    def test_nao_chama_apresentacao_se_atendimento_antigo(
        self, mock_filter, mock_msg_get, mock_features_compose
    ):
        mock_atendimento = MagicMock(spec=Atendimento, id=1)
        mock_mensagem = MagicMock(spec=Mensagem, atendimento=mock_atendimento)
...
@patch(
    "smart_core_assistant_painel.app.ui.oraculo.utils._obter_entidades_metadados_validas"
)
@patch("smart_core_assistant_painel.app.ui.oraculo.utils.FeaturesCompose")
class TestProcessarEntidadesContato:
    """Testes para a função _processar_entidades_contato."""

    def test_atualiza_nome_e_metadados(
        self, mock_features_compose, mock_obter_entidades
    ):
        mock_obter_entidades.return_value = {"cpf", "email"}
        mock_contato = MagicMock(
            spec=Contato, nome_contato="Antigo", metadados={}
        )
        mock_mensagem = MagicMock(
            spec=Mensagem,
            atendimento=MagicMock(spec=Atendimento, contato=mock_contato),
        )
    ...
    def test_nao_atualiza_nome_menor(
        self, mock_features_compose, mock_obter_entidades
    ):
        mock_obter_entidades.return_value = set()
        mock_contato = MagicMock(
            spec=Contato, nome_contato="NomeLongo", metadados={}
        )
        mock_mensagem = MagicMock(
            spec=Mensagem,
            atendimento=MagicMock(spec=Atendimento, contato=mock_contato),
