...
class TestTreinamentoService(TestCase):
    ...
    @patch("smart_core_assistant_painel.app.ui.oraculo.views.FeaturesCompose.pre_analise_ia_treinamento")
    def test_aplicar_pre_analise_documentos(self, mock_pre_analise):
        ...
        documentos_processados = TreinamentoService.aplicar_pre_analise_documentos(documentos)

        self.assertEqual(len(documentos_processados), 2)
        self.assertEqual(documentos_processados[0].page_content, "Conteúdo pré-analisado")
    ...
    def test_processar_arquivo_upload_sem_caminho_temporario(self, mock_tempfile):
    ...
    def test_processar_conteudo_texto(self, mock_features_compose):
        ...
        mock_features_compose.pre_analise_ia_treinamento.return_value = "Conteúdo pré-analisado"
        mock_features_compose.load_document_conteudo.return_value = [Document(page_content="doc")]

        docs = TreinamentoService.processar_conteudo_texto(1, "conteúdo", "tag", "grupo")

        self.assertEqual(len(docs), 1)
        mock_features_compose.pre_analise_ia_treinamento.assert_called_once_with("conteúdo")
    ...
    @patch("smart_core_assistant_painel.app.ui.oraculo.views.TreinamentoService.aplicar_pre_analise_documentos")
    @patch("smart_core_assistant_painel.app.ui.oraculo.views.FeaturesCompose")
    def test_processar_arquivo_documento(self, mock_features_compose, mock_aplicar_pre_analise):
        """Testa o processamento de um arquivo de documento."""
        mock_features_compose.load_document_file.return_value = [Document(page_content="doc")]
        mock_aplicar_pre_analise.return_value = [Document(page_content="doc_analisado")]

        docs = TreinamentoService.processar_arquivo_documento(1, "/path/to/doc.txt", "tag", "grupo")
        ...
        mock_aplicar_pre_analise.assert_called_once_with([Document(page_content="doc")])
...
class TestOraculoViews(TestCase):
    ...
    def test_treinar_ia_post_sem_tag_ou_grupo(self):
        ...
        response = self.client.post(reverse("oraculo:treinar_ia"), {"conteudo": "teste"}, follow=True)
    ...
    def test_treinar_ia_post_sem_conteudo_ou_documento(self):
        ...
        response = self.client.post(reverse("oraculo:treinar_ia"), {"tag": "t", "grupo": "g"}, follow=True)
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]), "É necessário fornecer conteúdo ou documento.")

    @patch("smart_core_assistant_painel.app.ui.oraculo.views._processar_treinamento")
    def test_treinar_ia_post_chama_processar(self, mock_processar):
        """Testa se a requisição POST chama a função de processamento."""
        mock_processar.return_value = HttpResponseRedirect(reverse("oraculo:treinar_ia"))
        self.client.post(reverse("oraculo:treinar_ia"), {"tag": "t", "grupo": "g", "conteudo": "c"})
        mock_processar.assert_called_once()

    @patch("smart_core_assistant_painel.app.ui.oraculo.views.TreinamentoService.limpar_arquivo_temporario")
    @patch("smart_core_assistant_painel.app.ui.oraculo.views.TreinamentoService.processar_conteudo_texto")
    @patch("smart_core_assistant_painel.app.ui.oraculo.views.TreinamentoService.processar_arquivo_documento")
    @patch("smart_core_assistant_painel.app.ui.oraculo.views.TreinamentoService.processar_arquivo_upload")
    @patch("django.db.transaction.atomic")
    def test_processar_treinamento_sucesso(self, mock_atomic, mock_upload, mock_proc_doc, mock_proc_texto, mock_limpar):
    ...
    @patch("smart_core_assistant_painel.app.ui.oraculo.views._exibir_pre_processamento")
    def test_pre_processamento_get(self, mock_exibir):
        """Testa a requisição GET para a view pre_processamento."""
        mock_exibir.return_value = HttpResponseRedirect(reverse("oraculo:treinar_ia"))
        self.client.get(reverse("oraculo:pre_processamento", args=[self.treinamento.id]))
        mock_exibir.assert_called_once()

    @patch("smart_core_assistant_painel.app.ui.oraculo.views._processar_pre_processamento")
    def test_pre_processamento_post(self, mock_processar):
        """Testa a requisição POST para a view pre_processamento."""
        mock_processar.return_value = HttpResponseRedirect(reverse("oraculo:treinar_ia"))
        self.client.post(reverse("oraculo:pre_processamento", args=[self.treinamento.id]), {"acao": "aceitar"})
        mock_processar.assert_called_once()
