...
class AnalisePreviaMensagemLangchainDatasource(APMData):
    def __call__(
        ...
        try:
            ...
            PydanticModel: Type[BaseModel] = create_dynamic_pydantic_model(
            ...
            is_local_model = (
                "local" in model_str
                or "llama" in model_str
                or "ollama" in model_str
                or "ollama" in llm_class_str
                or "chatollama" in llm_class_str
                or hasattr(llm, "_is_local")
                or "127.0.0.1" in model_str
                or "localhost" in model_str
            )

            logger.info(
                f"Tipo de modelo detectado: {'LOCAL' if is_local_model else 'REMOTO/CLOUD'}"
            )
            ...
            try:
                if is_local_model:
                    ...
                    try:
                        ...
                        logger.info(
                            "Usando método padrão de structured_output para modelo local"
                        )
                    except Exception as local_error:
                        # Se falhar, tentar com método JSON explícito
                        logger.warning(f"Erro ao usar método padrão: {local_error}")
                        structured_llm = llm.with_structured_output(
                            PydanticModel, method="json"
                        )
                        logger.info(
                            "Usando method=json para modelo local após falha no método padrão"
                        )
                else:
                ...
                logger.info(
                    "Usando LLM diretamente com instruções para formato JSON no prompt"
                )
            ...
            try:
                response: Union[BaseModel, str, dict[Any, Any], None] = chain.invoke(
                    invoke_data
                )
            ...
            if response is None:
                logger.warning(
                    "Resposta da chain é None, usando fallback com resposta vazia"
                )
                response = PydanticModel(intent=[], entities=[])
            ...
            # Verificar se a resposta é uma string (pode ocorrer com modelos locais)
                ...
                logger.warning(
                    f"Resposta da chain é uma string, tentando parsear como JSON: {response[:100]}..."
                )
                try:
                    import json
            ...
            if hasattr(response, "intent") and hasattr(response, "entities"):
                # Type guard para garantir que response é um BaseModel com os atributos corretos
                assert isinstance(response, BaseModel), (
                    f"Resposta deve ser BaseModel, recebido: {type(response)}"
                )
                ...
                intent_dicts = [
                    ...
                    for item in getattr(response, "intent", [])
                ]

                # Extrair entities como lista de dicionários {tipo: valor}
                entity_dicts = [
                    {str(item.type): item.value}
                    for item in getattr(response, "entities", [])
                ]
                ...
                raise ValueError(
                    f"Resposta inesperada: {
                        type(response)
                    }. Esperado: PydanticModel dinâmico"
                )
    ...
    def _formatar_historico_atendimento(
        ...
        Recebe sempre um dict[str, Any] conforme retornado pelo método
        carregar_historico_mensagens do modelo Atendimento.

        Args:
            historico_atendimento: Dict contendo:
        ...
        Returns:
            String formatada para o contexto do prompt
        """
        try:
            # Extrai as mensagens do histórico
            conteudo_mensagens = historico_atendimento.get("conteudo_mensagens", [])

            # Se não há mensagens, retorna indicação de ausência
            ...
            # Adiciona informações sobre intents e entidades se disponíveis
            intents_detectados = historico_atendimento.get("intents_detectados", set())
            ...
            if intents_detectados:
                historico_texto += (
                    f"\nINTENTS DETECTADOS: {', '.join(sorted(intents_detectados))}\n"
                )

            if entidades_extraidas:
                historico_texto += (
                    f"\nENTIDADES EXTRAÍDAS: {', '.join(sorted(entidades_extraidas))}\n"
                )
