...
class TestAnaliseConteudoLangchainDatasource:
    ...
    def llm_parameters(self):
        ...
        return LlmParameters(
            ...
            error=LlmError("Erro de teste"),  # Usando instância
        )
    ...
    @patch(
        "smart_core_assistant_painel.modules.ai_engine.features.analise_conteudo.datasource.analise_conteudo_langchain_datasource.ChatPromptTemplate"
    )
    def test_call_successful_string_response(
        self, mock_chat_prompt, datasource, llm_parameters
    ):
    ...
    def test_call_with_think_tags_removal(
        self, mock_chat_prompt, datasource, llm_parameters
    ):
...
final"""
    ...
    def test_call_with_whitespace_cleanup(
        self, mock_chat_prompt, datasource, llm_parameters
    ):
    ...
    def test_call_non_string_response_raises_type_error(
        self, mock_chat_prompt, datasource, llm_parameters
    ):
        ...
        with pytest.raises(
            TypeError,
            match="Resposta do LLM deve ser uma string, mas recebeu: <class 'dict'>",
        ):
    ...
    def test_call_none_response_raises_type_error(
        self, mock_chat_prompt, datasource, llm_parameters
    ):
        ...
        with pytest.raises(
            TypeError,
            match="Resposta do LLM deve ser uma string, mas recebeu: <class 'NoneType'>",
        ):
            datasource(llm_parameters)
    ...
    def test_prompt_template_construction(
        self, mock_chat_prompt, datasource, llm_parameters
    ):
    ...
    def test_llm_creation_and_chaining(
        self, mock_chat_prompt, datasource, llm_parameters
    ):
